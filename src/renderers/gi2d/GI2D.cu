...
void GI2DRenderer::OnInitialise()
{
    ...
    m_objects.pathTracer = CreateAsset<GI2D::Host::PathTracer>("id_gi2DPathTracer", m_objects.sceneBIH, m_objects.hostLineSegments, m_clientWidth, m_clientHeight, 3, m_renderStream);
...
void GI2DRenderer::OnRender()
{
    ...
    m_dirtyFlags = m_dirtyFlags.load() | kGI2DDirtyParams;
    ...
    if (m_dirtyFlags)
    {
        ...
        if (m_dirtyFlags & kGI2DDirtyParams)
        {
            ...
            m_objects.overlayParams.view = m_objects.viewTransform;
            m_objects.pathTracerParams.view = m_objects.viewTransform;
