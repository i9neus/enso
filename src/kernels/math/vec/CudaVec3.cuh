...
{		
	...
	struct __vec_swizzle<float, 3, 3, 0, 1, 2>
	{
		...
		__host__ __device__ inline explicit __vec_swizzle(const float v) : x(v), y(v), z(v) {}
		__host__ __device__ inline __vec_swizzle(const float& x_, const float& y_, const float& z_) : x(x_), y(y_), z(z_) {}
		__host__ __device__ inline __vec_swizzle(const vec2 & v, const float& z_) : x(v.x), y(v.y), z(z_) {}

        // Cast from other vec3 types
        template<typename OtherType, int I0, int I1, int I2>
        __host__ __device__ explicit __vec_swizzle(const __vec_swizzle<OtherType, 3, 3, I0, I1, I2>& v) :
		...
		__host__ __device__ inline __vec_swizzle(const __vec_swizzle<float, ActualSize, 3, In...> & swizzled) 
        { 
        ...
        __host__ __device__ inline __vec_swizzle& operator=(const __vec_swizzle<float, RS, 3, R0, R1, R2>& rhs)
        {
            x = rhs.other[R0]; y = rhs.other[R1]; z = rhs.other[R2];
            return *this;
        }

		__host__ __device__ inline __vec_swizzle& operator=(const float& v) { x = v; y = v; z = v; return *this; }
    ...
    __host__ __device__ inline vec3 operator +(const __vec_swizzle<float, LS, 3, L0, L1, L2>& lhs, const __vec_swizzle<float, RS, 3, R0, R1, R2>& rhs)
    {
        return { lhs.data[L0] + rhs.data[R0], lhs.data[L1] + rhs.data[R1], lhs.data[L2] + rhs.data[R2] };
    }
    template<int LS, int L0, int L1, int L2>
    __host__ __device__ inline vec3 operator +(const __vec_swizzle<float, LS, 3, L0, L1, L2>& lhs, const float& rhs)
    {
    ...
    template<int LS, int L0, int L1, int L2>
    __host__ __device__ inline vec3 operator -(const __vec_swizzle<float, LS, 3, L0, L1, L2>& lhs)
    {
    ...
    template<int RS, int R0, int R1, int R2>
    __host__ __device__ inline vec3 operator *(const float& lhs, const __vec_swizzle<float, RS, 3, R0, R1, R2>& rhs)
    {
    ...
    template<int LS, int L0, int L1, int L2, int RS, int R0, int R1, int R2>
    __host__ __device__ inline __vec_swizzle<float, LS, 3, L0, L1, L2>& operator +=(__vec_swizzle<float, LS, 3, L0, L1, L2>& lhs, const __vec_swizzle<float, RS, 3, R0, R1, R2>& rhs)
    {
    ...
    template<int LS, int L0, int L1, int L2>
    __host__ __device__ inline __vec_swizzle<float, LS, 3, L0, L1, L2>& operator *=(__vec_swizzle<float, LS, 3, L0, L1, L2>& lhs, const float& rhs)
    {
    ...
    // Vector functions
    template<int LS, int L0, int L1, int L2, int RS, int R0, int R1, int R2>
    __host__ __device__ inline float dot(const __vec_swizzle<float, LS, 3, L0, L1, L2>& lhs, const __vec_swizzle<float, RS, 3, R0, R1, R2>& rhs)
    {
        return lhs.data[L0] * rhs.data[R0] + lhs.data[L1] * rhs.data[R1] + lhs.data[L2] * rhs.data[R2];
    }
    template<int LS, int L0, int L1, int L2, int RS, int R0, int R1, int R2>
    __host__ __device__ inline vec3 cross(const __vec_swizzle<float, LS, 3, L0, L1, L2>& lhs, const __vec_swizzle<float, RS, 3, R0, R1, R2>& rhs)
    {
    ...
    template<int LS, int L0, int L1, int L2>
    __host__ __device__ inline float length2(const __vec_swizzle<float, LS, 3, L0, L1, L2>& v)
    {
        return v.data[L0] * v.data[L0] + v.data[L1] * v.data[L1] + v.data[L2] * v.data[L2];
    }
    template<int LS, int L0, int L1, int L2>
    __host__ __device__ inline float length(const __vec_swizzle<float, LS, 3, L0, L1, L2>& v)
    {
        return sqrt(v.data[L0] * v.data[L0] + v.data[L1] * v.data[L1] + v.data[L2] * v.data[L2]);
    }
    template<int LS, int L0, int L1, int L2>
    __host__ __device__ inline __vec_swizzle<float, LS, 3, L0, L1, L2> normalize(const __vec_swizzle<float, LS, 3, L0, L1, L2>& v)
    {
        return v / length(v);
    }
    template<int LS, int L0, int L1, int L2, int RS, int R0, int R1, int R2>
    __host__ __device__ inline vec3 fmod(__vec_swizzle<float, LS, 3, L0, L1, L2>& a, const __vec_swizzle<float, RS, 3, R0, R1, R2>& b)
    {
        return { fmodf(a.data[L0], b.data[R0]), fmodf(a.data[L1], b.data[R1]), fmodf(a.data[L2], b.data[R2]) };
    }
    template<int LS, int L0, int L1, int L2>
    __host__ __device__ inline vec3 fmod(__vec_swizzle<float, LS, 3, L0, L1, L2>& a, const float& b)
    {
        return { fmodf(a.data[L0], b), fmodf(a.data[L1], b), fmodf(a.data[L2], b) };
    }
    template<int LS, int L0, int L1, int L2, int RS, int R0, int R1, int R2>
    __host__ __device__ inline vec3 pow(__vec_swizzle<float, LS, 3, L0, L1, L2>& a, const __vec_swizzle<float, RS, 3, R0, R1, R2>& b)
    {
        return { powf(a.data[L0], b.data[R0]), powf(a.data[L1], b.data[R1]), powf(a.data[L2], b.data[R2]) };
    }
    template<int LS, int L0, int L1, int L2>
    __host__ __device__ inline vec3 exp(__vec_swizzle<float, LS, 3, L0, L1, L2>& a)
    {
        return { exp(a.data[L0]), exp(a.data[L1]), exp(a.data[L2]) };
    }
    template<int LS, int L0, int L1, int L2>
    __host__ __device__ inline vec3 log(__vec_swizzle<float, LS, 3, L0, L1, L2>& a)
    {
        return { logf(a.data[L0]), logf(a.data[L1]), logf(a.data[L2]) };
    }
    template<int LS, int L0, int L1, int L2>
    __host__ __device__ inline vec3 log10(__vec_swizzle<float, LS, 3, L0, L1, L2>& a)
    {
        return { log10f(a.data[L0]), log10f(a.data[L1]), log10f(a.data[L2]) };
    }
    template<int LS, int L0, int L1, int L2>
    __host__ __device__ inline vec3 log2(__vec_swizzle<float, LS, 3, L0, L1, L2>& a)
    {
        return { log2f(a.data[L0]), log2f(a.data[L1]), log2f(a.data[L2]) };
    }

    __host__ __device__ inline vec3 clamp(const vec3& v, const vec3& a, const vec3& b) { return { clamp(v.x, a.x, b.x), clamp(v.y, a.y, b.y), clamp(v.z, a.z, b.z) }; }
    __host__ __device__ inline vec3 saturate(const vec3& v, const vec3& a, const vec3& b) { return { clamp(v.x, 0.0f, 1.0f), clamp(v.y, 0.0f, 1.0f), clamp(v.z, 0.0f, 1.0f) }; }
    __host__ __device__ inline vec3 abs(const vec3& a) { return { fabs(a.x), fabs(a.y), fabs(a.z) }; }
    __host__ __device__ inline float sum(const vec3& a) { return a.x + a.y + a.z; }
    __host__ __device__ inline vec3 ceil(const vec3& v) { return { ceilf(v.x), ceilf(v.y), ceilf(v.z) }; }
    __host__ __device__ inline vec3 floor(const vec3& v) { return { floorf(v.x), floorf(v.y), floorf(v.z) }; }
    __host__ __device__ inline vec3 sign(const vec3& v) { return { sign(v.x), sign(v.y), sign(v.z) }; }
    ...
    //__host__ __device__ inline vec3 saturate(const vec3& v, const vec3& a, const vec3& b)	{ return { __saturatef(v.x), __saturatef(v.x), __saturatef(v.x)); }}
