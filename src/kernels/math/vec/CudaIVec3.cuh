...
namespace Cuda
{
	...
	struct __vec_swizzle<Type, 3, 3, 0, 1, 2>
	{
		...
		__host__ __device__ inline explicit __vec_swizzle(const Type v) : x(v), y(v), z(v) {}
		__host__ __device__ inline __vec_swizzle(const Type& x_, const Type& y_, const Type& z_) : x(x_), y(y_), z(z_) {}
		__host__ __device__ inline __vec_swizzle(const __ivec2<Type>& v, const Type& z_) : x(v.x), y(v.y), z(z_) {}
		...
		__host__ __device__ inline __vec_swizzle(const __vec_swizzle<Type, ActualSize, 3, In...>& swizzled)
		{
		...
		__host__ __device__ inline __vec_swizzle& operator=(const __vec_swizzle<Type, RS, 3, R0, R1, R2>& rhs)
		{
			x = rhs.other[R0]; y = rhs.other[R1]; z = rhs.other[R2];
			return *this;
		}

		__host__ __device__ inline __vec_swizzle& operator=(const Type& v) { x = v; y = v; z = v; return *this; }
    ...
    __host__ __device__ inline __ivec3<Type> operator +(const __vec_swizzle<Type, LS, 3, L0, L1, L2>& lhs, const __vec_swizzle<Type, RS, 3, R0, R1, R2>& rhs)
    {
    ...
    __host__ __device__ inline __ivec3<Type> operator +(const __vec_swizzle<Type, LS, 3, L0, L1, L2>& lhs, const Type& rhs)
    {
    ...
    template<typename Type, int LS, int L0, int L1, int L2>
    __host__ __device__ inline __ivec3<Type> operator -(const __vec_swizzle<Type, LS, 3, L0, L1, L2>& lhs)
    {
    ...
    __host__ __device__ inline __ivec3<Type> operator *(const Type& lhs, const __vec_swizzle<Type, RS, 3, R0, R1, R2>& rhs)
    {
    ...
    template<typename Type, int LS, int L0, int L1, int L2, int RS, int R0, int R1, int R2>
    __host__ __device__ inline __vec_swizzle<Type, LS, 3, L0, L1, L2>& operator +=(__vec_swizzle<Type, LS, 3, L0, L1, L2>& lhs, const __vec_swizzle<Type, RS, 3, R0, R1, R2>& rhs)
    {
    ...
    template<typename Type, int LS, int L0, int L1, int L2>
    __host__ __device__ inline __vec_swizzle<Type, LS, 3, L0, L1, L2>& operator *=(__vec_swizzle<Type, LS, 3, L0, L1, L2>& lhs, const Type& rhs)
    {
    ...
    template<typename Type, int LS, int L0, int L1, int L2>
    __host__ __device__ inline __ivec3<Type>& operator %=(__vec_swizzle<Type, LS, 3, L0, L1, L2>& lhs, const Type& rhs)
    {
        lhs.data[L0] %= rhs; lhs.data[L1] %= rhs; lhs.data[L2] %= rhs;  return lhs;
    }
    template<typename Type, int LS, int L0, int L1, int L2, int RS, int R0, int R1, int R2>
    __host__ __device__ inline __ivec3<Type>& operator %=(__vec_swizzle<Type, LS, 3, L0, L1, L2>& lhs, const __vec_swizzle<Type, RS, 3, R0, R1, R2>& rhs)
    {
        return { lhs.data[L0] % rhs.data[R0], lhs.data[L1] % rhs.data[R1], lhs.data[L2] % rhs.data[R2]};
    }
    template<typename Type, int LS, int L0, int L1, int L2>
    ...
    template<typename Type>
    __host__ __device__ inline __ivec3<Type> clamp(const __ivec3<Type>& v, const __ivec3<Type>& a, const __ivec3<Type>& b)
    {
        return { clamp(v.x, a.x, b.x), clamp(v.y, a.y, b.y), clamp(v.z, a.z, b.z) };
    }
    template<typename Type>
    __host__ __device__ inline __ivec3<Type> abs(const __ivec3<Type>& a)
    {
        return { abs(a.x), abs(a.y), abs(a.z) };
    }
    template<typename Type>
    __host__ __device__ inline Type sum(const __ivec3<Type>& a)
    {
        return a.x + a.y + a.z;
    } 
    template<typename Type>
    __host__ __device__ inline __ivec3<Type> sign(const __ivec3<Type>& v)
    {
        return { sign(v.x), sign(v.y), sign(v.z) };
    }

    template<typename Type>
    __host__ __device__ inline bool operator==(const __ivec3<Type>& lhs, const __ivec3<Type>& rhs)
    {
    ...
    template<typename Type>
    __host__ __device__ inline Type cwiseMax(const __ivec3<Type>& v)
    {
        return (v.x > v.y) ? ((v.x > v.z) ? v.x : v.z) : ((v.y > v.z) ? v.y : v.z);
    }

    template<typename Type>
    __host__ __device__ inline Type cwiseMin(const __ivec3<Type>& v)
    {
        return (v.x < v.y) ? ((v.x < v.z) ? v.x : v.z) : ((v.y < v.z) ? v.y : v.z);
    }
