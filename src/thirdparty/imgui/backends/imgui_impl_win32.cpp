...
namespace ImGui
{
    ...
    //  2021-02-18: Added ImplWin32_EnableAlphaCompositing(). Non Visual Studio users will need to link with dwmapi.lib (MinGW/gcc: use -ldwmapi).
    //  2021-02-17: Fixed ImplWin32_EnableDpiAwareness() attempting to get SetProcessDpiAwareness from shcore.dll on Windows 8 whereas it is only supported on Windows 8.1.
    //  2021-01-25: Inputs: Dynamically loading XInput DLL.
    //  2020-12-04: Misc: Fixed setting of io.DisplaySize to invalid/uninitialized data when after hwnd has been closed.
    //  2020-03-03: Inputs: Calling AddInputCharacterUTF16() to support surrogate pairs leading to codepoint >= 0x10000 (for more complete CJK inputs)
    //  2020-02-17: Added ImplWin32_EnableDpiAwareness(), ImplWin32_GetDpiScaleForHwnd(), ImplWin32_GetDpiScaleForMonitor() helper functions.
    ...
    static void ImplWin32_UpdateGamepads()
    {
#ifndef IMGUI_IMPL_WIN32_DISABLE_GAMEPAD
        ImGuiIO& io = ImGui::GetIO();
...
#undef MAP_ANALOG
    ...
    void    ImplWin32_NewFrame()
    {
        ImGuiIO& io = ImGui::GetIO();
        IM_ASSERT(io.Fonts->IsBuilt() && "Font atlas not built! It is generally built by the renderer backend. Missing call to renderer _NewFrame() function? e.g. ImplOpenGL3_NewFrame().");
...
// Copy this line into your .cpp file to forward declare the function.
    ...
    // ImplWin32_EnableDpiAwareness() is just a helper called by main.cpp, we don't call it automatically.
...
#define DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2 (DPI_AWARENESS_CONTEXT)-4
    ...
    void ImplWin32_EnableDpiAwareness()
    {
...
#pragma comment(lib, "gdi32")   // Link with gdi32.lib for GetDeviceCaps(). MinGW will require linking with '-lgdi32'
    ...
    float ImplWin32_GetDpiScaleForMonitor(void* monitor)
    {
    ...
    float ImplWin32_GetDpiScaleForHwnd(void* hwnd)
    {
        HMONITOR monitor = ::MonitorFromWindow((HWND)hwnd, MONITOR_DEFAULTTONEAREST);
        return ImplWin32_GetDpiScaleForMonitor(monitor);
    }
...
// (the Dwm* functions are Vista era functions but we are borrowing logic from GLFW)
    void ImplWin32_EnableAlphaCompositing(void* hwnd)
    {
