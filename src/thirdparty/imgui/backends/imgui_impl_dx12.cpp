...
//  2019-10-18: DirectX12: *BREAKING CHANGE* Added extra ID3D12DescriptorHeap parameter to ImplDX12_Init() function.
...
// DirectX data
    ...
    static void ImplDX12_SetupRenderState(ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx, FrameResources* fr)
    {
    ...
    // Render function
    void ImplDX12_RenderDrawData(ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx)
    {        
        ...
        ImplDX12_SetupRenderState(draw_data, ctx, fr);
    ...
    static void ImplDX12_CreateFontsTexture()
    {
        // Build texture atlas
    ...
    bool    ImplDX12_CreateDeviceObjects()
    {
        if (!g_pd3dDevice)
            return false;
        if (g_pPipelineState)
            ImplDX12_InvalidateDeviceObjects();
        ...
        ImplDX12_CreateFontsTexture();

        return true;
    }

    void    ImplDX12_InvalidateDeviceObjects()
    {
        if (!g_pd3dDevice)
            return;
    ...
    bool ImplDX12_Init(ID3D12Device* device, int num_frames_in_flight, DXGI_FORMAT rtv_format, ID3D12DescriptorHeap* cbv_srv_heap,
