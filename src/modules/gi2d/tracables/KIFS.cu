...
namespace Enso
{
    __host__ __device__ KIFSParams::KIFSParams()        
    {
        ...
        m_kifs.numIterations = 5;
        //m_kifs.objectBounds = BBox2f(vec2(-0.1), vec2(0.1));
        m_kifs.objectBounds = 0.2f;
    }

    __host__ __device__ Device::KIFS::KIFS() :
        m_kBary(vec2(1.0, -0.5 / 0.866025), vec2(0., 1. / 0.866025)),
        m_kBaryInv(vec2(1.0, 0.5), vec2(0., 0.866025)),
    ...
    __host__ __device__ __forceinline__ bool  Device::KIFS::Iterate(vec2 z, vec3& F) const
    { 
        if (length2(z) > m_kifs.objectBounds) { return false; }

        F.x = length(z) - m_kifs.isosurface;
        F.yz = (F.x >= 0.0f) ? z : -z;
        return true;
        ...
        F.x = length(z) * powf(m_kifs.iterScale, float(m_kifs.numIterations)) - m_kifs.isosurface;
        F.xy = (F.x >= 0.0f) ? z : -z;
        return true;
    }
