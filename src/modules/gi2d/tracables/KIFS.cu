...
namespace Enso
{
    ...
    __host__ AssetHandle<Host::GenericObject> Host::KIFS::Instantiate(const std::string& id, const Host::Asset& parentAsset, const AssetHandle<const Host::SceneContainer>& scene)
    {
        return AssetAllocator::CreateChildAsset<Host::KIFS>(parentAsset, id);
    }
