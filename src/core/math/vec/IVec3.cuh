...
namespace Enso
{
	...
	struct __vec_swizzle<Type, 3, 3, 0, 1, 2>
	{
        ...
        __host__ __device__ __forceinline__ __vec_swizzle& operator*=(const __vec_swizzle& rhs) { x *= rhs.x; y *= rhs.y; z *= rhs.z; return *this; }
        __host__ __device__ __forceinline__ __vec_swizzle& operator/=(const __vec_swizzle& rhs) { x /= rhs.x; y /= rhs.y; z /= rhs.z; return *this; }
        __host__ __device__ __forceinline__ __vec_swizzle& operator+=(const Type& rhs) { x += rhs; y += rhs; z += rhs; return *this; }
        __host__ __device__ __forceinline__ __vec_swizzle& operator-=(const Type& rhs) { x -= rhs; y -= rhs; z -= rhs; return *this; }
        __host__ __device__ __forceinline__ __vec_swizzle& operator*=(const Type& rhs) { x *= rhs; y *= rhs; z *= rhs; return *this; }
        __host__ __device__ __forceinline__ __vec_swizzle& operator/=(const Type& rhs) { x /= rhs; y /= rhs; z /= rhs; return *this; }
    ...
    template<typename Type>
    __host__ __device__ __forceinline__ Type cwiseExtremum(const __ivec3<Type>& v)
    {
    ...
    template<typename Type>
    __host__ __device__ __forceinline__ __ivec3<Type> max(const __ivec3<Type>& a, const __ivec3<Type>& b) { return __ivec3<Type>(::max(a.x, b.x), ::max(a.y, b.y), ::max(a.z, b.z)); }
    template<typename Type>
    __host__ __device__ __forceinline__ __ivec3<Type> min(const __ivec3<Type>& a, const __ivec3<Type>& b) { return __ivec3<Type>(::min(a.x, b.x), ::min(a.y, b.y), ::min(a.z, b.z)); }
