...
{		    
    ...
    struct __align__(16) __vec_swizzle<float, 4, 4, 0, 1, 2, 3>
	{
        ...
        __host__ __device__ __forceinline__ __vec_swizzle(const float4& v) : x(v.x), y(v.y), z(v.z), w(v.w) {}
        ...
        __host__ __device__ __forceinline__ __vec_swizzle& operator*=(const __vec_swizzle& rhs) { x *= rhs.x; y *= rhs.y; z *= rhs.z; w *= rhs.w; return *this; }
        __host__ __device__ __forceinline__ __vec_swizzle& operator/=(const __vec_swizzle& rhs) { x /= rhs.x; y /= rhs.y; z /= rhs.z; w /= rhs.w; return *this; }
        __host__ __device__ __forceinline__ __vec_swizzle& operator+=(const float& rhs) { x += rhs; y += rhs; z += rhs; w += rhs; return *this; }
        __host__ __device__ __forceinline__ __vec_swizzle& operator-=(const float& rhs) { x -= rhs; y -= rhs; z -= rhs; w -= rhs; return *this; }
        __host__ __device__ __forceinline__ __vec_swizzle& operator*=(const float& rhs) { x *= rhs; y *= rhs; z *= rhs; w *= rhs; return *this; }
        __host__ __device__ __forceinline__ __vec_swizzle& operator/=(const float& rhs) { x /= rhs; y /= rhs; z /= rhs; w /= rhs; return *this; }
