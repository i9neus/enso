...
namespace Enso
{
	...
	struct __align__(8) __vec_swizzle<float, 2, 2, 0, 1>
	{
        ...
        // Cast from built-in float4 type
        __host__ __device__ __forceinline__ __vec_swizzle(const float2& v) : x(v.x), y(v.y) {}

        // Cast from other vec types
        ...
        /*template<typename OtherType, int ActualSize, int... In>
        __host__ __device__ __forceinline__ __vec_swizzle(const __vec_swizzle<OtherType, ActualSize, 2, In...>& swizzled)
        {
            swizzled.UnpackTo<0, 1, In...>(data);
        }*/

        // Assign from swizzled types
        template<int OtherSize, int R0, int R1, int... In>
        __host__ __device__ __forceinline__ __vec_swizzle& operator=(const __vec_swizzle<float, OtherSize, 2, R0, R1, In...>& rhs)
        {
            x = rhs[R0]; y = rhs[R1];
            return *this;
        }
    ...
    __host__ __device__ __forceinline__ vec2 max(const vec2& a, const vec2& b) { return vec2(fmaxf(a.x, b.x), fmaxf(a.y, b.y)); }
    __host__ __device__ __forceinline__ vec2 min(const vec2& a, const vec2& b) { return vec2(fminf(a.x, b.x), fminf(a.y, b.y)); }
