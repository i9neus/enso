...
namespace Enso
{
    ...
    __host__ void SerialisableAttributeProperties::Finalise()
    {
        // If the labele is blank, set it to the ID
        if (m_uiWidget.label.empty()) { m_uiWidget.label = m_id; }
    }

    __host__ SerialisableObjectSchema::SerialisableObjectSchema(const std::string& schemaId, const Json::Node& node) :
        m_id(schemaId)
    {
        Deserialise(node);
    }

    __host__ const std::vector<std::string>& SerialisableObjectSchema::GetDataTypeStrings()
    {
    ...
    __host__ const std::vector<std::string>& SerialisableObjectSchema::GetWidgetTypeStrings()
    {
        static const std::vector<std::string> widgetTypeStr = { "default", "input", "drag", "slider" };
        return widgetTypeStr;
    }

    __host__ void SerialisableObjectSchema::Serialise(Json::Node& rootNode) const
    {
    ...
    __host__ void SerialisableObjectSchema::Deserialise(const Json::Node& node)
    {
        ...
        for (Json::Node::ConstIterator it = node.begin(); it != node.end(); ++it)
        {
            ...
            attr.Finalise();
            m_attributes.emplace_back(std::make_shared<SerialisableAttributeProperties>(attr));
        }
    }

    __host__ void SerialisableObjectSchemaContainer::Load(const std::string& jsonPath)
    {
    ...
    __host__ void SerialisableObjectSchemaContainer::RegisterSchema(const std::string& schemaId, const std::string& schema)
    {
    ...
    __host__ void SerialisableObjectSchemaContainer::RegisterSchema(const std::string& schemaId, const SerialisableObjectSchema& schema)
    {
        AssertMsgFmt(!FindSchema(schemaId), "Error: schema was object '%s' is already registered.", schemaId);
    ...
    // Unregister a schema
    __host__ void SerialisableObjectSchemaContainer::UnregisterSchema(const std::string& schemaId)
    {
    ...
    __host__ std::shared_ptr<const SerialisableObjectSchema> SerialisableObjectSchemaContainer::FindSchema(const std::string& schemaId)
    {
        auto it = m_serialSchemaMap.find(schemaId);
    ...
    __host__ bool SerialisableObjectSchemaContainer::SchemaExists(const std::string& schemaId)
    {
